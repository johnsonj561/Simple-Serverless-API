const deleteUser = require('./index').handler;
...
it('Returns 200 if user exists and delete is successful', (done) => {
  // get returns Item, user does exist
  ...
  deleteUser(event, context, (err, resp) => {
    const { statusCode } = resp;
    const body = JSON.parse(resp.body);
    expect(statusCode).toEqual(200);
    expect(body.data).toMatch('deleted');
    done();
  });
});
